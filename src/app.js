...
const getLibraryInfo = async (list) => {
	...
	const formatList = await Promise.all(list.map(async (item) => {
		...
		const existList = list.map((v, i) => {
			const { hasBook, loanAvailable } = results[i].data.response.error === undefined ? results[i].data.response.result : { hasBook: 'N', loanAvailable: 'N'}
			return {
				...v,
				hasBook,
				loanAvailable, 
...
const delBookFromList = (isbn, target) => {


}

const addBookToList = (isbn, e) => {
	const savedBooks = getBooks()
	const { target } = e
	const updatedBooks = savedBooks.map(book => {
		return book.isbn === isbn ? {...book, is_listed: true} : book
	})
	...
	document.body.addEventListener('click', (e) => {
		...
		if (target.classList.contains('reading_btn')) {
			...
			if (isDeleted) {
				...
				notyf.success('삭제되었습니다.')
			} else {
				target.innerText = `삭제하기`
				notyf.success('추가되었습니다.')
			} 
		}
	})
}
				...
				if (shouldUpdateList.length > 0) {
					getLibraryInfo(shouldUpdateList).then(response => {
						...
						response.forEach((v, i) => {
							const metabox = document.querySelector(`div[data-isbn='${v.isbn}']`)
							if (metabox !== undefined) {
								while (metabox.hasChildNodes()) {
									metabox.removeChild(metabox.lastChild)
								}
								metabox.appendChild(parseHTML(MetadataBox(response[i])))
							}
						})
					})
				}
        
				el.appendChild(Paginator(props))
			}
